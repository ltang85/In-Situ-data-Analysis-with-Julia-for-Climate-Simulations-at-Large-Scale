...
!
  ...
  public insitu_processing ! insitu subroutine !  
...
!! In situ processing entry point subroutine
!! Defined by Soumya Dutta (sdutta@lanl.gov)
!!------------------------------------------------------------------------------
subroutine insitu_processing(cam_out, phys_state, dyn_in, dyn_out)
   ...
   use cam_history,     only: outfld
   use cam_logfile, only: iulog
   use time_manager, only: get_nstep, is_last_step, is_first_step ! soumya added
   ...
   type (dyn_export_t), intent(inout) :: dyn_out ! Dynamics export container
   type (element_t), pointer :: elem(:)
   integer :: rc, ii, i, j, k, p, ie, tl_f
   type(index_t), pointer :: idx

   real(r8) :: time_step !soumya added
   character(len=*), parameter :: fname="fname_", lname=".txt"
   character(len=100) :: pid, fullname, makedirectory

   !! for julia integration
   integer :: elem_count
   integer :: arridx
   real(r8), dimension (:), allocatable :: data_array
   real(r8) :: lat_to_julia(nlev)
   real(r8) :: lon_to_julia(nlev)
   real(r8) :: uvel1_to_julia(nlev)
...
!!      print*, "^^^^^^^^^^^^^^^ ", iam
   ...
   !!! Julia testing code here
   !! 96 causes invalid memory reference for temp. This is needed only for this
   !specific small case.
   if (iam<96) then
      ...
      allocate( data_array(elem_count*4))
      ...
      do ie=1,nelemd
            ...
            do ii = 1, idx%NumUniquePts
                ...
                !print*, "here ", iam, arridx
                data_array(arridx) = dyn_in%elem(ie)%spherep(i,j)%lat*rad2deg
                data_array(arridx+1) = dyn_in%elem(ie)%spherep(i,j)%lon*rad2deg
                data_array(arridx+2) = elem(ie)%state%V(i,j,1,12,tl_f)
                data_array(arridx+3) = elem(ie)%state%V(i,j,1,13,tl_f)
                arridx = arridx + 4
            enddo
      enddo
      ...
      res_julia = jl_call_subroutine("streaming", data_array, 4*elem_count, iam, get_nstep(), mpicom, 9)
...
end subroutine insitu_processing
